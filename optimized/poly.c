...
void poly_uniform(poly *a, const unsigned char *seed) {
	...
	for (i = 0; i < RINGCT_N / 64; i++) /* generate a in blocks of 64 coefficients */
	{
		ctr = 0;
		extseed[RINGCT_SYMBYTES] = i; /* domain-separate the 16 independent calls */
		for (k = 0; k < SHA3_STATESIZE; ++k)
			state[k] = 0;
		SHA3_shake128_absorb(state, extseed, RINGCT_SYMBYTES + 1);
		while (ctr < 64) /* Very unlikely to run more than once */
		{
			SHA3_shake128_squeezeblocks(buf, 1, state);
			for (j = 0; j < SHA3_CSHAKE128_RATE && ctr < 64; j += 2) {
